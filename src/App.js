...
class App extends React.Component {
  ...
  render() {
    ...
    return (
      ...
      <Navbar hamburgerButton={this.hamburgerButtonClickHandler} />
      <MobileHamburgerPopout show={this.state.hamburgerOpen} />
      {backdrop}
        <Switch>
        <Route exact path="/" component={HomePage} />
        <Route path="/404" component={NotFound} />
        <Route path="/TweetFinder" component={TweetFinder} />
        <Route path="/RandomTweet" component={RandomTweet} />
        <Redirect to="/404" />
        </Switch>
    </div> </BrowserRouter>
    );
